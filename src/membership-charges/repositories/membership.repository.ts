...
export class MembershipRepository {
  ...
  async getMembershipOrThrow(id: string): Promise<PlayerMembershipWithRelations> {
  ...
  async getMembershipById(id: string): Promise<PlayerMembershipWithRelations | null> {
  ...
  async getActiveMembershipsIdsBySeason(teamSeasonId: string): Promise<{ id: string }[]> {
  ...
  async getMembershipsForDailyGeneration(evaluationDate: Date): Promise<PlayerMembershipWithRelations[]> {
    return this.prisma.playerMembership.findMany({
      where: {
        status: { in: [ PlayerMembershipStatus.ACTIVE, PlayerMembershipStatus.PENDING_ACTIVE, PlayerMembershipStatus.SUSPENDED ] },
        OR: [ { nextRecurringChargeGenerationDate: { lte: evaluationDate } }, { nextRecurringChargeGenerationDate: null } ],
        teamSeason: { season: { endDate: { gte: evaluationDate } } },
      },
      include: playerMembershipInclude,
    });
  }
