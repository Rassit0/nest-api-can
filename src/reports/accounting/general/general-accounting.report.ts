    ...
    this.registry.register(
      {
        ...
        description:
          'Muestra ingresos, egresos, movimientos internos, liquidez y obligaciones por moneda.',
        formats: ['pdf'],
        filters: ['dateRange'],
  ...
  async generate(params: any, format: string): Promise<any> {
    ...
    activeCurrencies.forEach((cur) => {
      ...
      builder.addExecutiveSummary([
        ...
        {
          label: `Resultado del Período (${cur})`,
          value: `${net.toFixed(2)} ${cur}`,
        },
        {
          label: `Liquidez Actual (${cur})`,
          value: `${liq.toFixed(2)} ${cur}`,
        },
      ]);
    });
    ...
    activeCurrencies.forEach((cur) => {
      ...
      if (incData) {
        if (incData.school > 0)
          incomeRows.push([
            'Ingresos de Escuela (Cursos/Matrículas)',
            `${incData.school.toFixed(2)}`,
          ]);
        if (incData.club > 0)
          incomeRows.push([
            'Ingresos de Club (Membresías)',
            `${incData.club.toFixed(2)}`,
          ]);
        ...
        if (incData.uncategorized > 0)
          incomeRows.push([
            'Ingresos Sin Clasificar',
            `${incData.uncategorized.toFixed(2)}`,
          ]);
      }
      ...
      if (expData) {
        ...
        if (expData.uncategorized > 0)
          expenseRows.push([
            'Egresos Sin Clasificar',
            `${expData.uncategorized.toFixed(2)}`,
          ]);
      }
    ...
    builder.addDataTable({
      ...
      rows: [
        [
          'Escuela / Estudiantes',
          `${receivables.receivables.expired.student.toFixed(2)}`,
          `${receivables.receivables.valid.student.toFixed(2)}`,
        ],
        [
          'Club / Membresías',
          `${receivables.receivables.expired.membership.toFixed(2)}`,
          `${receivables.receivables.valid.membership.toFixed(2)}`,
        ],
        [
          'Cuentas Generales',
          `${receivables.receivables.expired.general.toFixed(2)}`,
          `${receivables.receivables.valid.general.toFixed(2)}`,
        ],
        [
          'TOTAL POR COBRAR',
          `${receivables.receivables.expired.total.toFixed(2)}`,
          `${receivables.receivables.valid.total.toFixed(2)}`,
        ],
      ],
    });
    ...
    builder.addDataTable({
      ...
      rows: [
        [
          'Obligaciones a Proveedores / Otros',
          `${payables.payables.expired.toFixed(2)}`,
          `${payables.payables.valid.toFixed(2)}`,
        ],
      ],
    });
    ...
    // --- NOTAS ---
    builder.addNotes(
      'Nota: Las obligaciones actualmente se almacenan como importes sin una moneda explícita en el modelo de base de datos. Por esta razón, este reporte no asigna artificialmente una divisa a las CxC/CxP ni las mezcla con los saldos de liquidez por moneda.',
    );
    builder.addNotes(
      'Nota Adicional: La información de flujos (Ingresos, Egresos y Movimientos Internos) corresponde al período seleccionado. La posición de liquidez representa el estado actual al momento de emisión del reporte.',
    );
